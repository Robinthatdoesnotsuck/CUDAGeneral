...
__global__ void linear_regression(float* x1, float* y, float* m1, float* c) {
    int idx = threadIdx.x;
    float x1_val = x1[idx];
    //float x2_val = x2[idx];
    //float x3_val = x3[idx];
...
__global__ void predict(float* x1,  float* y_pred, float* m1, float* c) {
    int idx = threadIdx.x;
    float x1_val = x1[idx];
    //float x2_val = x2[idx];
    //float x3_val = x3[idx];

    y_pred[idx] = (*m1) * x1_val  + (*c);
}
...
int main() {
    ...
    for (int i = 0; i < train_n; i++) {
        //cout << "GDPS:" << h_x1[i] << endl;
        h_x1[i] = trainSet[i].GDP;
        //cout << "GDPS:" << h_x1[i] << endl;
    ...
    for (int i = 0; i < iterations; i++) {
        linear_regression << < 1, train_n >> > (d_x1, d_y, d_m1, d_c);
    ...
    predict << < 1, test_n >> > (d_x1_test, d_y_pred, d_m1, d_c);
    cudaDeviceSynchronize();
    ...
    cout << "Predicted Percentage: " << h_y_pred[0] << endl;
    cout << "Actual GDP:" << testSet[0].Percentage_E << endl;
